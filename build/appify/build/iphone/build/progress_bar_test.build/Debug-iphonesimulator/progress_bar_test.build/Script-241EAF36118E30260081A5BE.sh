...
if [ "x$TITANIUM_CLI_XCODEBUILD" == "x" ]; then
    /workspace/lib/node-v0.10.38-darwin-x64/bin/node "/usr/local/bin/ti" build --platform iphone --sdk "3.5.1.GA" --no-prompt --no-progress-bars --no-banner --no-colors --build-only --xcode
    exit $?
else
