...
//Rare exceptions to indicate a bug in the _progress_bar_test code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_progress_bar_test = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_progress_bar_test]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.progress_bar_test.module"
